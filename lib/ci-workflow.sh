...
detect_ci_tools() {
    ...
    if [[ -z "${NATIVE_CI_CMD:-}" ]]; then
        # 1. Si existe 'task ci' en el Taskfile raíz, ÚSALO.
        if [ -f "${root}/Taskfile.yaml" ] && grep -q "ci:" "${root}/Taskfile.yaml"; then
    ...
    if [[ -z "${LOCAL_PIPELINE_CMD:-}" ]]; then
        if [ -f "${root}/Taskfile.yaml" ] && grep -q "pipeline:local:" "${root}/Taskfile.yaml"; then
    ...
    if [[ -z "${ACT_CI_CMD:-}" ]]; then
        # 1. Si existe 'task ci:act' en el Taskfile raíz, ÚSALO.
        if [ -f "${root}/Taskfile.yaml" ] && grep -q "ci:act:" "${root}/Taskfile.yaml"; then

...
wait_for_workflow_success_on_ref_or_sha_or_die() {
    ...
    while true; do
        # Intento 1: buscar por branch/tag si se proveyó
        if [[ -n "${ref:-}" ]]; then
            run_id="$(
        ...
        # Intento 2: buscar general por SHA (si el ref no ha sido actualizado aún o es detach)
        if [[ -z "${run_id:-}" ]]; then
            run_id="$(
